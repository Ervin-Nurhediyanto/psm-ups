<template>
  <span v-if="i === 1">
    <span class="text-danger"><b>{{ type_op }}</b></span>
    {{ data.table_1st[i - 1][0][0] }}
    <span v-for="j in data.table_1st[i - 1].length - 3" :key="j">
      <!-- Operation -->
      <span v-if="j === data.table_1st[i - 1].length - 3" class="px-1">=</span>
      <span v-if="j > 1 && j < data.table_1st[i - 1].length - 3 && data.table_1st[i - 1][j + 1] >= 0" class="px-1">+</span>
      <!-- value variable -->
...
<script>
export default {
  name: 'Change-Destination-Function',
