  ...
  <div v-if="index === n" class="mx-1 mb-1">
    ...
    <div v-for="i in data.table_it[index - 3].length" :key="i">
      <div v-if="data.table_it[index - 3][i - 1][0][0] === 'Z'">
        ...
        <span>
          {{ Math.round(((data.table_it[index -3][i - 1][data.table_it[index - 3][i - 1].length - 2][0]) + Number.EPSILON) * 100) / 100 }}
        </span>
        <span v-if="Math.round(((data.table_it[index -3][i - 1][data.table_it[index - 3][i - 1].length - 2][1]) + Number.EPSILON) * 100) / 100 !== 0">
          <span v-if="Math.round(((data.table_it[index -3][i - 1][data.table_it[index - 3][i - 1].length - 2][1]) + Number.EPSILON) * 100) / 100 > 0">+</span>
          <span>
            {{ Math.round(((data.table_it[index -3][i - 1][data.table_it[index - 3][i - 1].length - 2][1]) + Number.EPSILON) * 100) / 100 }}M
          </span>
        </span>
      </div>
